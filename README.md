...
```yaml
TRACK_AIRPORT: "HGH"
OUTPUT_PATH: "special-livery-live.json"
...
推荐的外部云端定时器是 Cloudflare Workers Cron Trigger。`cloudflare/` 目录提供了 Worker 模板，每天 00:00 UTC 运行一次，也就是北京时间 08:00，然后调用 GitHub workflow dispatch API。
...
| 云端定时器 | Cloudflare Workers Cron 每天北京时间 08:00 触发 `workflow_dispatch`。 |
